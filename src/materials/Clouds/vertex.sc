...
void main() {
  ...
  vec3 worldPos;
    ...
    vec4 color;
      ...
      color.rgb = mix(vec3(0.42, 0.48, 0.62), vec3(1.0, 1.0, 1.03), vGrad);
      // sunlit-top rim boost so cloud crowns pop
      ...
      float fade = fog_fade(worldPos.xyz);
        ...
        vec3 approxViewDir = normalize(worldPos - ViewPositionAndTime.xyz);
        vec3 mainSunDir = env.sunDir.y > 0.0 ? env.sunDir : env.moonDir;
        vec3 sunTint = sunLightTint(env.dayFactor, env.rainFactor);
        color = renderCloudsSimple(skycol, cloudPos, t, rain, approxViewDir, mainSunDir, sunTint);
