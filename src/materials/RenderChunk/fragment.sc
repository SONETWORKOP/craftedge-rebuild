...
void main() {
  ...
  // ESTN-style sunbeams (suraj se radial kiranen): fogged outdoor din me.
  // Paani ke andar nahi (v_sunMoon.w), suraj upar ho tabhi (v_reflSun.y).
  #ifdef NL_BEAMS
    if (v_sunMoon.w < 0.5 && v_reflSun.y > 0.0) {
      float beamFacing;
      float beamPat = nlSunBeamPattern(v_position - CameraPosition.xyz, v_reflSun.xyz, beamFacing);
      float beamGate = v_fog.a * v_lightmapUV.y * clamp(v_reflSun.w, 0.0, 1.0);
