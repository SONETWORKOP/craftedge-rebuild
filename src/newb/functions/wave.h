...
void nlWave(
  ...
  bool isSpecialLeaves =
    ...
    (texN>=803 && texN<=804);   // Pale Oak Leaves
  bool leavesWave = isTreeLeaves || isSpecialLeaves;

  // Tree leaves wave even when not biome-colored (e.g. cherry/azalea leaves
  // use a neutral tint so isColored is false). Plants/vines still need color.
  bool shouldWave = ((leavesWave && !isLeafLitter) || ((isPlants || isVines) && isColored && !isLeafLitter)) || (isFarmPlant && isTop);
